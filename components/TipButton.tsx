...
export default function TipButton({ postId, authorName, onTipSuccess }: TipButtonProps) {
  ...
  const handleTip = async () => {
    ...
    try {
      ...
      console.log(`Tipped ${tipAmount} PC to ${authorName} for post ${postId}`)

      onTipSuccess(Number.parseFloat(tipAmount))
      setShowTipModal(false)

      // Show success message
      alert(`Successfully tipped ${tipAmount} PC to ${authorName}!`)
    } catch (error) {
      ...
      {showTipModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">Amount (PC)</label>
              <input
                type="number"
                step="0.001"
                min="0.001"
            ...
            <div className="flex gap-3">
              ...
              >
                {isLoading ? "Processing..." : `Tip ${tipAmount} PC`}
              </button>
            </div>
          </div>
        </div>
